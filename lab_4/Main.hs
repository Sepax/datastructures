...
addNeighbour s q e
  ...
  where
    ...
    m         = fromJust $ Q.findMin q          -- node with the lowest weight in q

    newEdge = Edge source neighbour (label e + label (Q.prio m)) -- new edge to be inserted into q
    oldEdge = Q.lookup neighbour q                               -- if neighbour already in q, get edge of neighbour, otherwise nothing
    
    minEdge = if oldEdge == Nothing                              -- compare edges and choose the one with the lowest weight
      then newEdge
      else min newEdge $ fromJust oldEdge
